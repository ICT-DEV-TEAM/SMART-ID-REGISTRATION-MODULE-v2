...
class LoginGUI():
    ...
    def __init__(self):
        ...
        self.loginFrame = ctk.CTkFrame(master=self.mainGui, fg_color=self.color.white, height=self.loginFrame_height, width=self.loginFrame_width)
        ...
        self.headerLogoLabel = ctk.CTkLabel(master=self.loginFrame, image=self.headerLogo, text='FCPC’s ID REGISTRATION', font=ctk.CTkFont(size=int(self.window_height * .0885), family="Inter"), text_color=self.color.black, compound="top")
        ...
        self.usernameLabel = ctk.CTkLabel(master=self.loginFrame, font=self.font, text="Username", text_color=self.color.black)
        self.usernameLabel.grid(row=2, column=1, sticky='ws', padx=self.paddingX, pady=self.paddingY)
        self.passwordLabel = ctk.CTkLabel(master=self.loginFrame, font=self.font, text="Password", text_color=self.color.black)
        self.passwordLabel.grid(row=3, column=1, sticky='ws', padx=self.paddingX, pady=self.paddingY)

        self.userEntry = ctk.CTkEntry(master=self.loginFrame, fg_color=self.color.very_soft_blue, width=int(0.6794 * self.loginFrame_width), height=int(0.1147 * self.loginFrame_height), border_width=0, corner_radius=5, font=self.font)
        self.userEntry.grid(row=2, column=2, padx=self.paddingX, pady=self.paddingY, columnspan=2)
        self.passEntry = ctk.CTkEntry(master=self.loginFrame, fg_color=self.color.very_soft_blue, width=int(0.6794 * self.loginFrame_width), height=int(0.1147 * self.loginFrame_height), border_width=0, corner_radius=5, font=self.font, show="*")
        self.passEntry.grid(row=3, column=2, padx=self.paddingX, pady=self.paddingY, columnspan=2)

        self.loginButton = ctk.CTkButton(master=self.loginFrame, fg_color=self.color.very_dark_blue, width=int(self.loginFrame_width * 0.403), height=int(self.loginFrame_height * 0.1325), text='LOGIN', font=self.font, text_color=self.color.white, command=self.login)
        self.loginButton.grid(row=4, column=2, pady=self.paddingY, padx=int((0.0225 * self.loginFrame_width)/2), sticky='e')

        self.clearButton = ctk.CTkButton(master=self.loginFrame, fg_color=self.color.dark_red, width=int(self.loginFrame_width * 0.2545), height=int(self.loginFrame_height * 0.1325), text='CLEAR', font=self.font, text_color=self.color.white, command=self.clear)
